...
const Auth = () => {
  ...
  const handleAuth = async (e: React.FormEvent) => {
    ...
    // Simple delay to show loading state, then redirect to home
    setTimeout(() => {
      if (isLogin) {
    ...
    <div className="min-h-screen flex items-center justify-center px-4 pt-20 pb-10">
      ...
      >
        ...
        <div className="text-center mb-8">
          ...
          >
            <img src="/bg removed logo.png" alt="ReBook Logo" className="w-16 h-16" />
            <span className="font-display text-3xl font-bold gradient-text">ReBook</span>
          </motion.div>
          <p className="text-muted-foreground text-sm">Reuse. Resell. Relearn.</p>
        </div>
          ...
          <AnimatePresence mode="wait">
            ...
            >
              {!isLogin && (
                <div className="relative">
                  ...
                  <Input
                    ...
                    className="pl-11 h-12 rounded-xl bg-muted/30 border-border/50 focus:border-primary/50"
                    required
                  />
                </div>
              )}
              ...
              <Button
                type="submit"
                disabled={loading}
                className="w-full h-12 rounded-xl text-base font-semibold bg-gradient-to-r from-primary to-emerald-400 hover:from-primary/90 hover:to-emerald-400/90 shadow-lg shadow-primary/25 transition-all duration-300"
              >
                {loading ? (
                  <motion.div
                    animate={{ rotate: 360 }}
